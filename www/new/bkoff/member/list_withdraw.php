...
#query
$sql1 = "select $column from $table  where cp_id='$CP_ID' $filter";			//자료수
...
//-->
	...
	<form name="fmSearch_user" method="get">
	...
	<select name="target" target="target" class='select'>
	<?=option_str($selectTxt,$selectValue,$target)?>
	</select>

	<span class="top"><input class="box" type="text" name="keyword" id="keyword" maxlength="40" value='<?=$keyword?>'></span>
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
          ...
          <td align="center"><a class="soft" href="view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>&<?=$basicLink?>" onFocus="blur(this)" title="상세정보보기"><?=user_id($rs[id])?></a></td>
          <td align="left"><?=titleCut(strip_tags($rs[content]),100)?></td>
          <td align="center"><?=$rs[reg_date]?></td>
	    </tr>
		<tr><td colspan="6" class='bar'></td></tr>
		<?=$error[noData]?>
<?
	$num--;
}
?>
        ...
        <tr><td colspan="6"  bgcolor='#FFFFFF' height=10></td></tr>
        <tr>
		  <td colspan="6">
