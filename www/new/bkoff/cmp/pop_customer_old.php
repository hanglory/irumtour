...
$sql = "
    select 
        ...
        phone
    from $table 
        where name like '%$name%' or leader like '%$name%' $filter 
...
if(strstr("@112.172.15.90@221.154.110.119@","@".$_SERVER["REMOTE_ADDR"]."@")){
    $sql = "
            ...
            from cmp_people
                ...
                and sex<>'$rs[sex]'
                and name_eng<>'$rs[name_eng]'
                and rn<>'$rs[rn]'
                and passport_no<>'$rs[passport_no]'
                and phone<>'$rs[phone]'
            group by name,sex,name_eng,rn,passport_no,phone
        ";
    list($rows)=$dbo->query($sql);
    ...
    while($rs=$dbo->next_record()){
        ...
        ?>
        <tr height="35" align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
          <td><span class="leader_<?=$j?>">x<?=$rs[leader]?></span></td>
          <td><span class="name_<?=$j?>">x<?=$rs[name]?></span></td>
          <td><span class="sex_<?=$j?>"><?=$rs[sex]?></span></td>
          <td><span class="name_eng_<?=$j?>"><?=$rs[name_eng]?></span></td>
          <td><span class="rn_<?=$j?>"><?=$rs[rn]?></span></td>
          <td><span class="passport_no_<?=$j?>"><?=$rs[passport_no]?></span></td>
