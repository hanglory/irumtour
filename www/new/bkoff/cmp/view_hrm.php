...
//승인권한
$proof_power = 0;
if($_SESSION["sessLogin"]["id"]=="sanha" || $_SESSION["sessLogin"]["id"]=="chadori") $proof_power = 1;
if($_SESSION["sessLogin"]["staff_type"]=="partner_a") $proof_power=1;
...
if ($mode=="save"){
	...
	$sql = "delete from $table where id_no = $id_no $FILTER_PARTNER_QUERY";
	$dbo->query($sql);
	$rs=$dbo->next_record();
	...
	$today=date("Y/m/d");
	if($proof_power){
		$sql = "update $table set proof='$today' where id_no = $id_no $FILTER_PARTNER_QUERY";
		$dbo->query($sql);
	}
	back();
	exit;
	...
	$rs= $dbo->next_record();
	$rs[staff_] = $rs[staff];
    if($rs[cp_id]==$CP_ID) $proof_power=1;
}
...
<div style="padding:0 10px 0 10px">
    ...
    <table border="0" cellspacing="1" cellpadding="3" width="97%">
        ...
        <tr>
          ...
          <td>
	           <?=radio($ASSORT_HRM,$ASSORT_HRM,$rs[assort],'assort')?>
          </td>
        </tr>
