...
//-->
	...
	*/
	...
	$dbo->query($sql);
	$rs=$dbo->next_record();
	if($debug) checkVar($rs[id_no].mysql_error(),$sql);
	...
	$rs[ins] = $rs2[ins];
	if($debug) checkVar($rows."/".$rs2[ins].mysql_error(),$sql2);
	...
	$dbo2->query($sql2);
	$rs2=$dbo2->next_record();
	if($debug) checkVar($rs2[add1].mysql_error(),$sql2);
	?>
