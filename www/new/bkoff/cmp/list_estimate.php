...
$dbo->query($sql_2);
if($debug){checkVar($LOGIN_STAFF_TYPE.mysql_error(),$sql_2);}
while($rs=$dbo->next_record()){



    $sql3="select * from cmp_ex_link where code='$rs[code]' and limit_date>'$today'";
    $dbo3->query($sql3);
    $rs3=$dbo3->next_record();
    $url1 = $rs3[url];
    ...
    if(!$url1) $url1 = short_url($long_url1);
?>
        <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
          ...
          <td>
              <?if($user_id=="tester" || $user_id=="test"){?>
                <?=$bit_copy?>
              <?}else{?>
              <a href="javascript:newWin('view_reservation.php?id_no=<?=$id_no_reservation?>',1200,650,1,1,'','reservation')" style="<?=$css?>"><?=$bit_copy?></a>
              <?}?>
          </td>
          ...
          <td>
            <?if($rs[plan_type]){?>
            <span class="btn_pack medium bold"><a href="javascript:newWin('form06.html?id_no=<?=$rs[id_no]?>',950,650,1,1,'','reservation<?=$rs[id_no]?>')">일정표</a>
            </span>
            <?}else{?>
          ...
          <td> 
            <?if($url1){?>
                ...
                <span class="btn_pack medium bold"><a href="javascript:newWin('<?=$long_url1?>',1000,8000,1,1)"> 바로가기 </a></span>
            <?}?>
          </td>
        </tr>
<?
