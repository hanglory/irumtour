...
.red{color:red;font-weight:bold}
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
       ...
       <input type=hidden name=mode value='drop'>
		...
		<th class="subject" colspan="3">출금내역</th>
		<th class="subject" rowspan="2">수수료<br/>(고객입금액-입금가)</th>
...
while($rs=$dbo->next_record()){
	...
	if($rs[d_date]<="2017/12/31"){
		...
		@$gain_one = ($rs[price_prev]-($rs3[price_air]+$rs3[price_land]))/$rs[people];
	}
...
?>
		...
		<td><?=$rs[d_date]?></td>
		<td><?=$night?></td>
		<td class="numberic"><?=nf($rs3[price])?><!--매출/인원수*1인판매가--></td>
		<td class="numberic"><?=nf($rs3[price_air])?></td>
		<td class="numberic"><?=nf($rs3[price_land])?></td>
		<td class="numberic red"><?=nf($rs3[price_air]+$rs3[price_land])?></td>
		<td class="numberic"><?=nf($rs[price_prev]-($rs3[price_air]+$rs3[price_land]))?><!-- 수수료 --></td>
		<td class="numberic"><?=nf($gain_one)?><!-- 1인수익 --></td>
		<td class="numberic red"><?=nf($rs[price_prev])?><!-- 고객입금액 --></td>
		<td class="numberic"><?=nf($rs[price_tmp_output])?><!--가지급금  --></td>
		<td><?=$staff[0]?></td>
		<td><?=$rs[pay_method]?></td>
		<td class="numberic"><?=nf($rs[payed_price])?><!-- 입금액 --></td>
...
<?
	...
	$price6+=($rs[price_prev]-($rs3[price_air]+$rs3[price_land]));
	$price7+=$gain_one;
...
?>
		...
		<td></td>
		<td></td>
		<td class="numberic" style="font-weight:bold;color:red"><?=nf($price12)?></td>
