...
#query
$sql_1 = "select $column from $table where id<>'' $filter";			//자료수
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      ...
	      <td><?=$num?></td>
	      <td><?=color($bit_login)?></td>
	      <td><?=$rs[id]?></td>
	      <td><a class=soft href="view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>&<?=$basicLink?>" onFocus="blur(this)" title="상세정보보기"><?=$rs[name]?></a></td>
	      <td><?=$rs[ip]?></td>
	      <td><?=$rs[last_login]?></td>
	      <td><?=$rs[reg_date]?></td>
	    </tr>
