...
if(strstr("@14.37.242.84@221.154.110.119@","@".$_SERVER["REMOTE_ADDR"]."@")){
    $url = str_replace("pop_reservation_apis.php","pop_reservation_apis_j.php",$CPAGE);
...
// $sql = "update cmp_reservation set price_last = price-(price_prev+price_prev2+price_prev3)";
...
if ($mode=="save"){
    ...
    $return_url = SELF."?code=${code}&apis=${apis}&people=${people}&leader=${leader}&tour_date=${tour_date}";
    msggo("저장하였습니다.",$return_url);
    exit;
...
<div style="padding:10px">
    ...
    <table border="0" cellspacing="0" cellpadding="0" width="100%" id="tbl_list">
        ...
        $sql = "select * from $table where code=$code and bit=1 order by seq asc";
        list($rows)=$dbo->query($sql);

        $bit_id_nos="";
        $rows_for = ($people>$rows)? $people : $rows;
        //for($j=1; $j <= $people; $j++){
        for($j=1; $j <= $rows_for; $j++){
            $rs=$dbo->next_record();
            ...
            $sql2 = "update $table set seq=$j where code=$code and bit=1 and id_no=$rs[id_no] and seq<999 limit 1";
            $dbo2->query($sql2);


        ?>
        <tr height="35" align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" class="bc_<?=$rs[bit_cancel]?>">
            <td class="">
            ...
            <td class="apis1"><input type="text" name="phone[]" id="phone_<?=$j?>" value="<?=$rs[phone]?>" size="13" maxlength="30" class="box" onchange="cell('phone_<?=$j?>',this.value)" /></td>
            ...
            <td class="apis1">
                ...
                <span class="btn_pack small bold"><a href="#" onClick="drop(<?=$j?>)">X</a></span>
            </td>
            ...
            <td class="apis2">
                ...
                <input type="hidden" id="bank_in1_id_<?=$j?>" name="bank_in1_id[]" value="<?=$rs[bank_in1_id]?>">
            </td>
            <td class="apis2">
            ...
            <td class="apis2">
                ...
                <input type="hidden" id="bank_in2_id_<?=$j?>" name="bank_in2_id[]" value="<?=$rs[bank_in2_id]?>">
            </td>
            <td class="apis2">
            ...
            <td class="apis2">
                ...
                <input type="hidden" id="bank_in3_id_<?=$j?>" name="bank_in3_id[]" value="<?=$rs[bank_in3_id]?>">
            </td>
            <td class="apis2">
