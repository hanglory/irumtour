...
.red{color:red;font-weight: bold}	
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
		...
		<tr onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
			...
			<td class="numberic <?=$color?>"><?=nf($rs[price_air])?></td><!-- 출금액1 -->
			<td class="numberic <?=$color?>"><?=nf($rs[price_land])?></td><!-- 출금액2 -->
			<td class="numberic <?=$color?>"><?=nf($rs[price_refund])?></td><!-- 출금액3 -->
		...
		<tr style="background-color:#ffe6cc">
			<td colspan="6" class="subject">합계</td>
		...
		<tr style="background-color:#ffe6cc">
			<td colspan="6" class="subject"><?=($pay_fix=="T")?"처리":"미처리"?> 합계</td>
	...
	<div style="padding:10px 0 10px 10px;float:right">
		...
		<span class="btn_pack medium bold"><a href="list_<?=$filecode?>_excel.php?date_s=<?=$date_s?>&date_e=<?=$date_e?>&target=<?=$target?>&pay_fix=<?=$pay_fix?>&keyword=<?=$keyword?>"> 엑셀 다운로드 </a></span>&nbsp;
		<?}?>
	</div>
