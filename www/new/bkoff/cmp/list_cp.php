...
####검색 항목
$selectTxt = "업체명,아이디,담당자,연락처";
$selectValue ="company,id,staff,staff_phone";
...
function del(){
	...
	if(j == 0){
		alert("삭제할 자료을 선택하지 않으셨습니다.");
		return;
	}
	if(confirm("선택한 자료을 삭제하시겠습니까?")){
...
//-->
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
       ...
       <input type=hidden name=mode value='drop'>
        ...
        <th class="subject" >파트너종류</th>
		<th class="subject" >업체명</th>
		<th class="subject" >담당자</th>
		<th class="subject" >담당자번호</th>
		<th class="subject" >링크</th>
        <!-- <th class="subject" >결제링크</th> -->
...
while($rs=$dbo->next_record()){
    ...
    $url  = ($rs[cp_url])? $rs[cp_url] : "http://${rs[id]}.irumplace.com";
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
          ...
          <td>
            <?
            if($rs[partner_type]=="partner_i") echo "파트너I";
            elseif($rs[partner_type]=="partner_a") echo "파트너A";
            elseif($rs[partner_type]=="partner_g") echo "파트너G";
            ?>
            <span style="color:red"><?=($rs[bit_hide])?"(비활성화)":""?></span>
          </td>
	      <td><a class=soft href="view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>&<?=$basicLink?>" onFocus="blur(this)" title="상세정보보기"><?=$rs[company]?></a></td>
	      <td><?=$rs[staff]?></td>
	      <td><?=$rs[staff_phone]?></td>
	      <td>
            <a href="<?=$url?>" target="_blank"><?=$url?></a>
          </td>
          <!-- <td><?if($rs[PTN_PG_API_ID] && $rs[PTN_PG_API_KEY]){?><a href="http://<?=$rs[id]?>.irumplace.com" target="_blank">http://<?=$rs[id]?>.irumplace.com/payment/</a><?}?></td> -->
