...
/*
$sql = "
CREATE TABLE cmp_set_price_golf (
...
if($mode=="save"){
	...
	if(!$id_no){
		$sql = "
			select * from cmp_set_price_golf
			where
				date_s<='$date_e'
				and
				date_e>='$date_s'
	...
	$sqlInsert="
		insert into cmp_set_price_golf (
           cp_id,
		   golf_id_no,
		   date_s,
		   date_e,
...
elseif($mode=="drop"){
	$sql = "delete from cmp_set_price_golf where id_no=$id_no and golf_id_no=$golf_id_no";
	$dbo->query($sql);
	//checkVar(mysql_error(),$sql);exit;
	back();
	exit;
...
<div style="padding:0 10px 0 10px;">
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="97%" id="tbl_normal">
	    ...
	    <?
		$sql = "select * from cmp_set_price_golf where golf_id_no=$golf_id_no and id_no=$id_no";
		$dbo->query($sql);
		$rs=$dbo->next_record();
		?>
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="97%" id="tbl_normal">
		...
		<?
		$sql = "select * from cmp_set_price_golf where golf_id_no=$golf_id_no order by date_s desc,date_e desc ";
		$dbo->query($sql);
