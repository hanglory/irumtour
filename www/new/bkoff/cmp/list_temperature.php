...
#query
$sql_1 = "select $column from $table where nation<>'' $filter $FILTER_PARTNER_QUERY";			//자료수
...
$m1 = date("m");
$m2  =date("m",mktime(0,0,0,date("m")+1,date("d"),date("Y")));
$m3  =date("m",mktime(0,0,0,date("m")+2,date("d"),date("Y")));
...
//-->
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
       ...
       <input type=hidden name=mode value='drop'>
		...
		<th class="subject"><?=$m1?>월</th>
		<th class="subject"><?=$m2?>월</th>
		<th class="subject"><?=$m3?>월</th>
		</tr>
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      ...
	      <td><?=$rs[nation]?></td>
	      <td><a href="javascript:newWin('view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>',870,600,1,1,'','air')"><?=$rs[city]?></a></td>
	      <td><?=titleCut2($rs[$col1],30)?><??></td>
	      <td><?=titleCut2($rs[$col2],30)?></td>
	      <td><?=titleCut2($rs[$col3],30)?></td>
	    </tr>
<?
	$num--;
}
        ...
        <tr>
		  ...
		  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="right">
			 <tr>
			  ...
			  <td align="right">
				<span class="btn_pack medium bold"><a href="javascript:newWin('view_<?=$filecode?>.php?ctg1=<?=$ctg1?>',870,600,1,1,'air')"> 등록 </a></span>&nbsp;
