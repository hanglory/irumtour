...
?>
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
       ...
       <input type=hidden name=mode value='drop'>
		...
		<th class="subject" width="50%">상담내역</th>
...
$sql = "select * from cmp_qna where name like '%$name%' $filter order by id_no desc";
$dbo->query($sql);
...
?>
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
       ...
       <input type=hidden name=mode value='drop'>
		...
		<th class="subject" width="40%">관리자메모</th>
		</tr>
<?
$name = trim($keyword);
if($name) $filter ="name like '%$name%' or golf_name like '%$name%' ";
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      <td height="25"><a href="javascript:newWin('form06.html?id_no=<?=$rs[id_no]?>',950,650,1,1,'','reservation<?=$rs[id_no]?>')"><?=$rs[send_date]?></a></td>
	      <td><?=$rs[golf_name]?></td>
	      <td><?=$rs[people]?></td>
	      <td><?=$rs[staff]?></td>
	      <td><?=nf($rs[price])?></td>
	      <td class="l" style="padding-left:10px"><?=$rs[memo]?></td>
	    </tr>
<?
	$num--;
}
...
?>
	    <tr align='center'>
			<?if(!$k){?>
	        <td rowspan="<?=$cnt?>" height="25"><a href="javascript:newWin('view_reservation.php?id_no=<?=$rs[id_no]?>',870,650,1,1,'','reservation')"><?=$rs[name]?></a></td>
			<td rowspan="<?=$cnt?>" width="7%"><?=$rs[phone]?></td>
			...
			<td rowspan="<?=$cnt?>" width="10%"><?=$rs[golf_name]?></td>
			<td rowspan="<?=$cnt?>" width="13%"><?=$rs[memo]?></td>
			<td rowspan="<?=$cnt?>" width="10%"><?=$rs[memo_payment]?></td>
			<?}?>
			<td><?=$rs2[name]?></td>
			<td><?=$rs2[phone]?></td>
			<td><?=$rs2[memo]?></td>
			<td><?=nf($rs2[price])?></td>
			<td><?=nf($rs2[price]-($rs2[price_air]+$rs2[price_land]))?></td>
