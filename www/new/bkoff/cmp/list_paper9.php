...
if($_SESSION["sessLogin"]["staff_type"] == "ceo" && !$partner){
    ...
    $sql = "select * from cmp_staff where staff_type in ('partner_i')";
    $dbo->query($sql);
    while($rs=$dbo->next_record()){
        $partners .="or main_staff like '%($rs[id])%' ";
    }
...
.r{padding-right:5px !important}
    ...
    */
    ...
    <?
    $sql = "delete from cmp_tmp_paper9";
    $dbo->query($sql);
    ...
    while($rs=$dbo->next_record()){
        ...
        $rs[sum_people] = ($rs[sum_people])? $rs[sum_people]: 0;
        $rs[sum_fee] = ($rs[sum_fee])? $rs[sum_fee]: 0;

        $DATA[$did][$did2]["people"] = $rs[sum_people];
        $DATA[$did][$did2]["fee"] = $rs[sum_fee];
        $arr[] = $rs[did];

        $danga = $rs[sum_fee]/$rs[sum_people];
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
        ...
        ?>
        <tr>
            <td style="background-color:#f0f0f0"><?=$rs[did]?></td>
            ...
            <td class="r" style="background-color:#F7F7F6"><?=@nf($rs[total_people])?></td>
            <td class="r" style="background-color:#F7F7F6"><?=@nf($rs[total_fee]/$rs[total_people])?></td>
        </tr>
       <?}?>
