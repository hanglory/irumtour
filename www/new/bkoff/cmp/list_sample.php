...
#query
$sql_1 = "select $column from $table where nation<>'' $filter $FILTER_PARTNER_QUERY";
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      <td height="35"><?if($bit_edit_power){?><input type="checkbox" name="check[]" value="<?=$rs[id_no];?>"></a><?}?></td>
	      <td>
                <?if($bit_edit_power){?>
                    <a href="javascript:newWin('view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>',870,400,1,1,'','air')"><?=$rs[nation]?></a>
                <?}else{?>
                    <a href="javascript:alert('직접 등록한 자료만 수정할 수 있습니다.')"><?=$rs[nation]?></a>
                <?}?>
          </td>
	      <td align="left" style="padding:10px">
                <?if($bit_edit_power){?>
                    <a href="javascript:newWin('view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>',870,400,1,1,'','air')">
                        <img src="../../public/cmp/<?=($rs[filename1])?>" width="300" onerror="this.style.display='none'">
                    </a>
                <?}else{?>
                    <a href="javascript:alert('직접 등록한 자료만 수정할 수 있습니다.')">
                        <img src="../../public/cmp/<?=($rs[filename1])?>" width="300" onerror="this.style.display='none'">
                    </a>
                <?}?>
