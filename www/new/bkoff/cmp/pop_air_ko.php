...
if($totalCount==1){
	...
	while(list($key,$val)=each($arr[body][items][item])){
		if($rs[assort]=="D"){
			...
			elseif($key=="arrivalcity") $rs[arrivalcity] = $val;
	    ...
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      ...
	      <td>
	      	<?if($rs[assort]=="D" && $rs[d_air_no]){?>
	      		<span class="btn_pack medium bold"><a href="javascript:set_air_api('d','<?=$code?>')"> 선택 </a></span>
	      	<?}?>
	      </td>
	      <td><?=$rs[r_air_no]?></td>
	      <td><?=$rs[r_time_s]?></td>
	      ...
	      <td>
	      	<?if($rs[assort]=="R" && $rs[r_air_no]){?>
	      		<span class="btn_pack medium bold"><a href="javascript:set_air_api('r','<?=$code?>')"> 선택 </a></span>
	      	<?}?>
	      </td>
	    </tr>
...
<?
	...
	while(@list($key,$val)=each($arr[body][items][item])){
		...
		while(list($key2,$val2)=@each($val)){
			//if(strstr("@112.172.15.90@221.154.110.119@","@".$_SERVER["REMOTE_ADDR"]."@"))checkVar($key2,$val2);

			if($rs[assort]=="D"){
				if($key2=="startcity") $rs[airport_in] = $val2;
				elseif($key2=="arrivalcity") $rs[arrivalcity] = $val2;
		...
		//if(strstr("@112.172.15.90@221.154.110.119@","@".$_SERVER["REMOTE_ADDR"]."@")){echo checkVar("",$code);}
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      <td style="height:35px">한국<?//=$rs[assort]?></td>
	      <td><?=$rs[airport_in]?></td>
