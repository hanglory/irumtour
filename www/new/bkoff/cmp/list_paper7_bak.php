...
//-->
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
		<td class="title_con"><img src="../images/common/ic_title.gif" align="absmiddle"><?=$TITLE?> (작업 전 파일)
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
	    ...
	    <tr align="center" height="25" bgcolor="#F7F7F6"'>
			<th class="subject" width="14%">대표자명</th>
			<th class="subject" width="14%">고객명</th>
			<th class="subject" width="14%">담당자</th>
			<th class="subject" width="14%">항공요금</th>
			<th class="subject" width="14%">환불예정액</th>
			<th class="subject" width="14%">환불액</th>
		...
		while($rs= $dbo->next_record()){
			...
			$sql2 = "select * from cmp_people where code='$rs[code]' and bit=1 and name<>'' order by seq asc";
		...
		<tr style="background-color:#fff">
			...
			<td class="r" style="padding-left:5px">
				<input type="text" name="return2_<?=$code2?>" id="return2_<?=$code2?>" value="<?=nf($PRICE2[$code2])?>" size="30" maxlength="30" class="box numberic w100" onchange="rprice_save('<?=$code2?>')"/>
			</td>

			<td class="r" style="padding-left:5px">
				<input type="text" name="return_<?=$code2?>" id="return_<?=$code2?>" value="<?=nf($PRICE[$code2])?>" size="30" maxlength="30" class="box numberic w100" onchange="rprice_save('<?=$code2?>')" style="color:red"/>
			</td>
			<td style="padding:0 5px 0 5px">
				<input type="text" name="memo_<?=$code2?>" id="memo_<?=$code2?>" value="<?=$MEMO[$code2]?>" size="30" maxlength="30" class="box w100" onchange="rprice_save('<?=$code2?>')"/>
			</td>
		</tr>
		<?
				$j++;
		...
		<tr style="background-color:#fff">
			...
			<td class="r" style="padding-left:5px">
				<input type="text" name="return_<?=$code2?>" id="return_<?=$code2?>" value="<?=nf($PRICE[$code2])?>" size="30" maxlength="30" class="box numberic w100"  style="color:red"  zonchange="rprice_save('<?=$code2?>')"/>
			</td>
			<td style="padding:0 5px 0 5px">
				<input type="text" name="memo_<?=$code2?>" id="memo_<?=$code2?>" value="<?=$MEMO[$code2]?>" size="30" maxlength="30" class="box w100" onchange="rprice_save('<?=$code2?>')"/>
			</td>
