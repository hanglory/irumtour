...
$sql = "select * from cmp_tour where id_no>0 $filter order by nation asc,city asc, name asc";
...
<!--
function set_data(id_no,name){

	opener.document.getElementById('tour<?=$bit?>_id_no').value=id_no;
	opener.document.getElementById('tour<?=$bit?>_name').value=name;
	opener.document.getElementById('tour<?=$bit?>').value=name;
...
<div style="padding:0 10px 0 10px">
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_list">
	    ...
	    <tr height="30" align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      ...
	      <td><?=$rs[name]?></td>
	      <td>
			<span class="btn_pack medium bold"><a href="javascript:set_data('<?=$rs[id_no]?>','<?=$rs[name]?>')"> 선택 </a></span>
		  </td>
	    </tr>
