...
####검색 항목
$selectTxt = "고객명,핸드폰,주소,발송내역,발송일,비고";
$selectValue ="name,cell,address,content,send_date,etc";
...
$dbo->query($sql_2);
//if($_SERVER["REMOTE_ADDR"]=="106.246.54.27") checkVar(mysql_error(),$sql_2);
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      ...
	      <td><?=$rs[save_date]?></td>
		  <td><a href="javascript:newWin('view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>&ctg1=<?=$ctg1?>',870,400,1,1,'golf')"><?=$rs[name]?></a></td>
	      <td><?=$rs[cell]?></td>
	      <td style="text-align:left;padding-left:10px"><?=$rs[address]?></td>
	      <td style="text-align:left;padding-left:10px"><?=titleCut2($rs[content],30)?></td>
	      <td><?=$rs[send_date]?></td>
	      <!-- <td><?=$rs[reg_date]?></td> -->
	    </tr>
<?
		  ...
		  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="right">
			 <tr>
			  ...
			  <td align="right">
				...
				<span class="btn_pack medium bold"><a href="list_<?=$filecode?>_excel.php?target=<?=$target?>&keyword=<?=$keyword?>"> 엑셀 </a></span>&nbsp;
