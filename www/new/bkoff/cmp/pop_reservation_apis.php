...
<div style="padding:10px">
    ...
    <table border="0" cellspacing="0" cellpadding="0" width="100%" id="tbl_list">
        ...
        <th class="subject apis3" >발권</th>
        <th class="subject apis3" >환불</th>
        ...
        for($j=1; $j <= $rows; $j++){
            ...
            $passport_no = str_replace("@","+",$rs[passport_no_]);
            $sql2 = "select * from cmp_passport where passport_no = '$passport_no' and passport_no<>'' and cp_id='$CP_ID' limit 1";
            $dbo2->query($sql2);
            $rs_tmp=$dbo2->next_record();
            if(!$rs_tmp[filename1]){
                $sql2 = "select * from cmp_passport where id_code='$id_code' and cp_id='$CP_ID' limit 1";
                $dbo2->query($sql2);
                $rs_tmp=$dbo2->next_record();
            }
            $photo_rows = (@is_file($_SERVER[DOCUMENT_ROOT]."/new/public/cmp_pass/".$rs_tmp[filename1]))?1:0;;
        



            //if(strstr("@14.37.242.84@221.154.110.119@","@".$_SERVER["REMOTE_ADDR"]."@")){checkVar($photo_rows.mysql_error(),$sql2);}
        ...
        <tr height="35" align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" class="bc_<?=$rs[bit_cancel]?> <?=($bit_birth_hide)?'hide':''?>">
            ...
            <td class="apis2">
                <input type="text" name="price_prev[]" id="price_prev_<?=$j?>" value="<?=nf($rs[price_prev])?>" size="8" maxlength="11" class="red box numberic" onkeyup="num('price_prev_<?=$j?>',this.value)" onclick="get_bank_data('i','price_prev_<?=$j?>',<?=$j?>,1)"/>
                <input type="hidden" id="bank_in1_id_<?=$j?>" name="bank_in1_id[]" value="<?=($rs[price_prev])?$rs[bank_in1_id]:""?>">
            </td>
            <td class="apis2">
                <input type="text" name="date_in[]" id="date_in_<?=$j?>" value="<?=$rs[date_in]?>" size="9" maxlength="10" class="box dateinput" onkeyup="num('date_in_<?=$j?>',this.value)"/>
            </td>
            <td class="apis2">
                <input type="text" name="price_prev2[]" id="price_prev2_<?=$j?>" value="<?=nf($rs[price_prev2])?>" size="8" maxlength="11" class="red box numberic" onkeyup="num('price_prev2_<?=$j?>',this.value)" onclick="get_bank_data('i','price_prev2_<?=$j?>',<?=$j?>,2)"/>
                <input type="hidden" id="bank_in2_id_<?=$j?>" name="bank_in2_id[]" value="<?=($rs[price_prev2])?$rs[bank_in2_id]:""?>">
            </td>
            <td class="apis2">
                <input type="text" name="date_in2[]" id="date_in2_<?=$j?>" value="<?=$rs[date_in2]?>" size="9" maxlength="10" class="box dateinput" onkeyup="num('date_in2_<?=$j?>',this.value)"/>
            </td>
            <td class="apis2">
                <input type="text" name="price_prev3[]" id="price_prev3_<?=$j?>" value="<?=nf($rs[price_prev3])?>" size="8" maxlength="11" class="red box numberic" onkeyup="num('price_prev3_<?=$j?>',this.value)" onclick="get_bank_data('i','price_prev3_<?=$j?>',<?=$j?>,3)"/>
                <input type="hidden" id="bank_in3_id_<?=$j?>" name="bank_in3_id[]" value="<?=($rs[price_prev3])?$rs[bank_in3_id]:""?>">
            </td>
            <td class="apis2">
                <input type="text" name="date_in3[]" id="date_in3_<?=$j?>" value="<?=$rs[date_in3]?>" size="9" maxlength="10" class="box dateinput" onkeyup="num('date_in3_<?=$j?>',this.value)"/>
            </td>
            ...
            <td class="apis2">
                <input type="hidden" name="insure[]" id="insure<?=$j?>" value="<?=($rs[insure])?1:0?>">
                <input type="checkbox" class="bit_ins" <?=($rs[insure])?"checked":""?> style="border:0;" onclick="if(this.checked==true){$('#insure<?=$j?>').val(1);}else{$('#insure<?=$j?>').val(0);}"/>
            </td>
