...
if($cp_pg_mode) $cp_id = $_SESSION["sessLogin"]["cp_id"];
...
#query
$sql1 = "select $column from $table  $filter and cp_id='$CP_ID'";          //자료수
...
if($CP_ID){
    ...
    $url_payment  = ($rs3[cp_url])? $rs3[cp_url]."/payment/" : "http://${rs3[id]}.irumplace.com/payment/";
}
//$url_payment = ($cp_id)? str_replace("//","//${cp_id}.",$url_irum_payment):$url_irum_payment;
//if(strstr("@211.226.255.74@221.154.110.119@","@".$_SERVER["REMOTE_ADDR"]."@")){checkVar("$cp_id",$url_payment);}
//-------------------------------------------------------------------------------
?>
<?include("../top.html");?>
...
?>
        <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" class="lh">
          ...
          <td align="left" style="padding-left:10px"><?=$url_payment?>?oid=<?=$rs[oid]?></td>
...
?>
    ...
    <strong>! 결제링크</strong> : <?=$url_payment?>
    </div> -->
