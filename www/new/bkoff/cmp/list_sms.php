...
//if(strstr("@112.172.15.90@221.154.110.119@","@".$_SERVER["REMOTE_ADDR"]."@")){checkVar("",$_SESSION["sessLogin"]["proof"]);}
...
if($mode=="sms"){
	...
	while(list($key,$val)=each($check)){
		...
		if($rs[origin_id_no]){//견적서에서 연결된 경우
			$sql2 = "select * from cmp_estimate where id_no=$rs[origin_id_no]";
			$dbo2->query($sql2);
			$rs2=$dbo2->next_record();			
			if($rs2[subject]) $goods_name=$rs2[subject];
			//if(strstr("@112.172.15.90@221.154.110.119@",$_SERVER["REMOTE_ADDR"])){checkVar(mysql_error(),$sql2);}
		}
		...
		if($ctg1==2){
			...
			$message = str_replace("{귀국일}",$r_date,$message);

		}elseif($ctg1==3){

			if(strstr($city,"제주")) $price_prev_one=200000;
			elseif($nation=="한국") $price_prev_one=150000;
			else $price_prev_one=300000;
			$price_prev_one = nf($price_prev_one);
	...
	//if(strstr("@112.172.15.90@221.154.110.119@",$_SERVER["REMOTE_ADDR"])){exit;}
	error("발송하였습니다.");
	exit;
}
		  ...
		  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="right">
			 <tr>
			  <td width="60%" align="left">
				...
				<span class="btn_pack medium bold"><a href="<?=SELF?>?ctg1=3&send_type=alim&talk_mode=payment_"> 계약금안내(알림톡) </a></span>&nbsp;
