...
?>
		<table width="<?=$page_width?>" border="0" cellspacing="1" cellpadding="0" style="border-collapse:collapse;border:1px solid #ccc ">
		  ...
		  <tr>
			...
			if($gname[$j]){ $golf_name_next = $gname[$j]; $gh_next=$gh[$j];$ah_next=$ah2;}
			else{ $golf_name_next=$gname[0]; $gh_next=$gh[0];$ah_next=$ah1;}
			?>


			<?
			if(!$plan_text[$i]){
				$contents[$i] = "조식 후 체크아웃 or 골프장으로 이동. $gh_next 소요 \n";
				$contents[$i] .= "[$golf_name_next 18홀 라운드] \n";
