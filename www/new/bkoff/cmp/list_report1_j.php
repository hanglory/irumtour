...
if($target!="b.partner"){
	$sql_1 = "
		select
			a.*,
			(select count(*) from cmp_people where code=a.code and price_air>0 and date_out='') as blank_date1,
			(select count(*) from cmp_people where code=a.code and price_land>0 and date_out2='') as blank_date2,
			(select count(*) from cmp_people where code=a.code and price_refund>0 and date_out3='') as blank_date3,
			(select count(*) from cmp_people where code=a.code and price_prev>0 and date_in='') as blank_date4,
			(select count(*) from cmp_people where code=a.code and price_prev2>0 and date_in2='') as blank_date5,
			(select count(*) from cmp_people where code=a.code and price_prev3>0 and date_in3='') as blank_date6,
...
?>
		...
		<td style="<?=$css?>"><?=trim($golf_name[0])?></td>
		...
		<td style="<?=$css?>" class="numberic red"><?=nf($rs[price_prev])?><!-- 고객입금액 --></td>
		...
		<td style="<?=$css?>" class="numberic"><?=($rs[bit_close])?"Y":"<span style='color:#ccc'>N</span>"?></td>
	    </tr>
<?
