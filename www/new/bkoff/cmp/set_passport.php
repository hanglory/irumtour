...
switch($mode){
	case "save":
		...
		if($upfile1){
			...
			$passport_limit = strtoupper(date("dMY",strtotime(substr($passport_limit,0,4)."/".substr($passport_limit,4,2)."/".substr($passport_limit,-2))));
			...
			if($rn){
                ...
                if(strlen($rn_)>13){
        			...
        			$rn2 =($sex=="M")? $rn_."1111111": $rn_."2222222";
                }
			}
      ...
      if(!$passport_no) $passport_no = "error_".time();
			$sql="
		 ...
		 if($mode2=="apis"){
			 echo "
				<script>
						opener.document.getElementById('name_${j}').value='$name';
						opener.document.getElementById('name_eng_${j}').value='$name_eng';
						opener.document.getElementById('rn_${j}').value='$rn2';
						opener.document.getElementById('passport_no_${j}').value='$passport_no2';
						opener.document.getElementById('passport_limit_${j}').value='$passport_limit';
				</script>
			 ";
		 }
	...
	case "drop":
		...
		$sql = "select * from cmp_passport where id_no = $id_no";
		$dbo->query($sql);
		$rs=$dbo->next_record();
		if($rs[filename1]) @unlink("../../public/cmp_pass/$rs[filename1]");

		$sql = "delete from cmp_passport where id_no = $id_no";
		$dbo->query($sql);
	...
	default:
		...
		$sql = "select * from cmp_passport where passport_no = '$passport_no' and passport_no<>'' and cp_id='$CP_ID' limit 1";
		$dbo->query($sql);
		$rs = $dbo->next_record();
        if(!$rs[id_no]){
            $sql = "select * from cmp_passport where id_code='$id_code' and cp_id='$CP_ID' limit 1";
            $dbo->query($sql);
            $rs = $dbo->next_record();
        }
...
function drop()
{
	var url="<?=SELF?>?mode=drop&code=<?=$code?>&id_no=<?=$rs[id_no]?>&j=<?=$j?>";
	location.href=url;
...
<div style="padding:0">
	...
	<?}else{?>
		...
		<table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_list">
			...
			<input type="hidden" id="customer_id_no" name="customer_id_no" value='<?=$customer_id_no?>'>
			<input type="hidden" id="name" name="name" value='<?=$name?>'>
			<input type="hidden" id="ocr" name="ocr" value='<?=($name=="OCR등록")?"ocr":""?>'>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right">
		 <tr>
		  ...
		  <td align="right" style="padding-right:23px">
			<span class="btn_save"><span class="btn_pack medium bold"><a href="javascript:chkForm()"> 저장 </a></span>&nbsp;&nbsp;&nbsp;</span>
