...
$sql = "select * from cmp_golf2 where id_no>0 $filter order by nation asc,city asc, name asc";
...
function set_data(id_no,name){
	...
	opener.document.getElementById('<?=$id?>_id_no').value=id_no;
	opener.document.getElementById('<?=$id?>_name').value=name;
	opener.document.getElementById('<?=$id?>').value=name;
