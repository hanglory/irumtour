...
####검색 항목
$selectTxt = "대표자,경로,발송일,담당자,예약일,출국일,귀국일,골프장명,연락처";
$selectValue ="name,view_path,send_date,main_staff,tour_date,d_date,r_date,golf_name,phone";
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      ...
	      <td><a href="javascript:newWin('view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>',950,650,1,1,'','estimate')"><?=$rs[name]?></a></td>
	      <td><?=$rs[view_path]?></td>
	      <td><?=$rs[main_staff]?></td>
	      <td><?=$rs[send_date]?></td>
	      <td><?=$rs[d_date]?></td>
	      ...
	      <td><?=$rs2[partner]?></td>
	      <td><?=nf($rs[price])?></td>
	      <td><span class="btn_pack medium bold"><a href="javascript:newWin('form06.html?id_no=<?=$rs[id_no]?>',950,650,1,1,'','reservation')">견적서</a></span>
		  </td>
		  ...
		  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="right">
			 <tr>
			  ...
			  <?if(strstr($_SESSION["sessLogin"]["proof"],"엑셀다운로드")){?>
				<span class="btn_pack medium bold"><a href="list_estimate_excel.php?target=<?=$target?>&keyword=<?=$keyword?>"> 엑셀 다운로드 </a></span>&nbsp;
			  <?}?>
				<span class="btn_pack medium bold"><a href="javascript:newWin('view_<?=$filecode?>.php?ctg1=<?=$ctg1?>',950,650,1,1,'','estimate')"> 등록 </a></span>&nbsp;
