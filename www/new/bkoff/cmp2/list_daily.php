...
<?
$name = trim($keyword);
$sql = "select * from cmp_reservation where tl>='$rs[date_s]' and tl<='$rs[date_e]' and bsp<>'' order by id_no desc ";
$dbo->query($sql);
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      <td height="25"><a href="javascript:newWin('view_reservation.php?id_no=<?=$rs[id_no]?>',870,650,1,1,'','reservation')"><?=$rs[tl]?></a></td>
	      <td><?=$rs[name]?></td>
	      <td><?=$rs[bsp]?></td>
	    </tr>
<?
...
<?
$name = trim($keyword);
$sql = "select * from cmp_reservation where d_date>='$date_s'  and d_date<='$date_e' order by d_date desc ";
$dbo->query($sql);
//checkVar(mysql_error(),$sql);
while($rs=$dbo->next_record()){
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      <td height="25"><a href="javascript:newWin('view_reservation.php?id_no=<?=$rs[id_no]?>',870,650,1,1,'','reservation')"><?=$rs[d_date]?></a></td>
	      <td><?=$rs[name]?></td>
	      <td><?=$rs[people]?></td>
	      <td><?=$rs[golf_name]?></td>
	      <td><?=$rs[d_air_no]?></td>
...
$arr = explode("/",$date_e); $date_e2 = date("Y/m/d",mktime(0,0,0,$arr[1],$arr[2]-1,$arr[0]));
//checkVar($date_s,$date_s2);
$sql = "select * from cmp_reservation where tour_date>='$date_s2'  and tour_date<='$date_e2' order by d_date desc ";
$dbo->query($sql);
//checkVar(mysql_error(),$sql);
$total1=0;
$total2=0;
...
//checkVar($date_s,$date_s2);
$sql = "select * from cmp_reservation where d_date>='$date_s2'  and d_date<='$date_e2' and bit_sending='Y' order by d_date desc ";
$dbo->query($sql);
//checkVar(mysql_error(),$sql);
while($rs=$dbo->next_record()){
