...
switch($mode){
	case "save":
		if(!$id_no){
			...
			$sql = "select max(seq)+1 as seq from $table where category1='$category1' ";
			$dbo->query($sql);
			$rs=$dbo->next_record();
			$seq= $rs[seq];
			$seq= ($seq)?$seq:1;
		...
		if($dbo->query($sql)){
			...
			if($id_no){
				$sql = "update ez_goods set category2='$category2',category_name=replace(category_name,'>${category2_old}>','>${category2}>') where category2='$category2_old'";
				$dbo->query($sql);
				$sql = "update ez_category2 set category2='$category2' where category2='$category2_old'";
				$dbo->query($sql);
			}
...
function chkForm(fm){
	...
	if(check_blank(fm.category2,'중분류를',0)=='wrong'){return }
...
<?
$sql2= "select * from ez_category1 order by seq asc";
$dbo2->query($sql2);
...
?>
		...
		<tr>
          <td height="20" class="subject">*<b>중분류</b></td>
          <td height="20">
