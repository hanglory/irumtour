...
//-->
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_cmp_list">
       ...
       <input type=hidden name=mode value='drop'>
		...
		<th class="subject blue" >출금액1,2(지상/항공요금)</th>
		<th class="subject red" >입금액1,2(계약금/잔금)</th>
		</tr>
<?
$sum1=0;
$sum2=0;
...
while($rs=$dbo->next_record()){
	...
	$sum1 += $rs[price_air]+$rs[price_land];
	$sum2 += $rs[price_prev]+$rs[price_prev2];
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">

	      <td height="30"><?=$rs[leader]?></td>
	      <td height="30"><?=$rs[name]?></td>
	      <td class="r pr10 blue"><?=nf($rs[price_air]+$rs[price_land])?></td>
	      <td class="r pr10 red"><?=nf($rs[price_prev]+$rs[price_prev2])?></td>
	    </tr>
<?
	$num--;
}
