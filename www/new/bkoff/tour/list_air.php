...
####검색 항목
$selectTxt = "항공사,출발지,도착지,출발일,도착일";
$selectValue ="company,place1,place2,period_s,period_e";
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_list">
       ...
       <input type=hidden name=mode value='drop'>
	    ...
	    <tr align=center height="30" bgcolor="#F7F7F6">
			...
			<td class="subject">항공사</td>
			<td class="subject">출발</td>
			<td class="subject">도착</td>
			<td class="subject">성인가격</td>
			<td class="subject">청소년가격</td>
			<td class="subject">소인가격</td>
          </tr>
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" class="lh">
	      <td><input type="checkbox" name="check[]" value="<?=$rs[code];?>"></td>
          <td align="center"><?=$num?></td>
          <td align="center"><a class=soft href="view_<?=$filecode?>.php?id_no=<?=$rs[id_no]?>&<?=$basicLink?>" onFocus="blur(this)" title="상세정보보기"><?=$rs[company]?></a></td>
          <td align="center"><?=$rs[place1]?> <?=$rs[period_s]?> <?=$rs[period_s2]?>:<?=$rs[period_s3]?></td>
          <td align="center"><?=$rs[place2]?> <?=$rs[period_e]?> <?=$rs[period_e2]?>:<?=$rs[period_e3]?></td>
          <td align="center"><?=number_format($rs[price_adult])?></td>
          <td align="center"><?=number_format($rs[price_child])?></td>
          <td align="center"><?=number_format($rs[price_baby])?></td>
	    </tr>
