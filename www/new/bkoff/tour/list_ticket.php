...
$MENU = "tour";
$TITLE = "할인입장권";
$filter = " where bit=1 and category1='ticket' ";
...
function set_position(str,bit){
	...
	if(confirm("설정하시겠습니까?")){
		...
		fm.mode2.value="list_ticket.php";
...
//-->
    ...
    <table border="0" cellspacing="0" cellpadding="3" width="100%" id="tbl_list">
       ...
       <input type=hidden name="ctg1" value="<?=$ctg1?>">
		...
		<td class="subject"><b>표시가격(대/청/소)</b></td>
		<td class="subject"><b>출발일</b></td>
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	      ...
	      <td><?=number_format($rs[price_adult])?> / <?=number_format($rs[price_youth])?> / <?=number_format($rs[price_child])?></td>
	      <td><span class="btn_pack small bold"><a href="javascript:mng_calendar(<?=$rs[tid]?>)"> 출발일 </a></span></td>
	      <!-- <td><span class="btn_pack small bold"><a href="javascript:copy(<?=$rs[copy]?>)"> 복사 </a></span></td> -->
	    </tr>
        <tr><td colspan="12" class='bar'></td></tr>
<?
	$num--;
