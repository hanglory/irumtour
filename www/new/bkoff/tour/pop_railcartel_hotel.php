...
#query
$sql1 = "select * from ez_tour where left(category1,2)='7-' $filter";			//자료수
$sql2 = $sql1 . " order by category1 asc,subject asc limit  $start, $view_row";

If($bit){
	$sql1 = "select a.* from ez_tour as a right join ez_railcartel_part as b on a.tid=b.sub_tid where b.assort='hotel' and b.tid=$tid $filter";			//자료수
	$sql2 = $sql1 . " order by a.category1 asc,a.subject asc limit  $start, $view_row";
}
...
$(function(){
	$(".choice").click(function(){
		...
		url = "pop_railcartel_hotel.php?mode=save";
		url += "&assort=hotel";
		url += "&tid="+tid;
		url += "&sub_tid="+sub_tid;
		url += "&bit="+bit;
	...
	$("#bit").click(function(){
		...
		url = "pop_railcartel_hotel.php?tid="+tid;
		url += "&bit="+bit;
...
//-->
	...
	<br>
		...
		<label><input type="checkbox" id="bit" value="1" <?=($bit)?"checked='checked'":""?> /> 선택된 호텔만 보기 </label>
		&nbsp;&nbsp;&nbsp;
	   ...
	   <table border="0" cellspacing="1" cellpadding="3" width="95%" align="center">
	    ...
	    <tr>
          ...
          <td class="subject l">카테고리</td>
          <td class="subject c">호텔명</td>
	    ...
	    <tr align="center">
			...
			<td class="l"><?=str_replace("여행","",get_category_name($rs[category1]))?></td>
			<td class="c"><a class="soft" href="javascript:newWin('view_tour.php?id_no=<?=$rs[id_no];?>&ctg1=<?=$ctg1_while?>',870,700,1,1)"><?=$rs[subject]?></a></td>
	...
	<?
	$sql = "select * from ez_tour_hotel_list where tid='$tid' group by tid,hotel,hotel_go,hotel_return,updown,station order by id_no asc";
	$dbo->query($sql);
	while($rs=$dbo->next_record()){
	?>

	<div class='tbl_title'>
		<img src="/images/view/ic_stitle.gif" alt=""/><?=$rs[station]?> / <?=($rs[updown]=="up")?"상행":"하행"?> / 가는편: <?=$rs[hotel_go]?>, 오는편: <?=$rs[hotel_return]?>
	</div>
	...
	<?
	$sql2 = "select * from ez_tour_hotel where tid='$tid' and hotel='$rs[hotel]' and hotel_go='$rs[hotel_go]' and hotel_return='$rs[hotel_return]' and updown='$rs[updown]' and station = '$rs[station]' order by period_s asc, period_e asc";
	$dbo2->query($sql2);
