...
if ($mode=="save"){
		...
		if($sub_code){
			$sql  ="delete from ez_hotel_room_jeju where code=$code and sub_code=$sub_code";
			$dbo->query($sql);
		}
		...
		for($i=0; $i <count($subject);$i++){
			...
			$sql="
			   insert into ez_hotel_room_jeju (
				  code,
				  sub_code,
				  subject,
				  period_s,
	...
	$sql = "delete from $table where code=$code and sub_code = $sub_code";
	$dbo->query($sql);

	redirect2("?code=$code&code=$code");exit;


}else{
	$sql = "select * from ez_hotel where code=$code";
	$dbo->query($sql);
	$rs= $dbo->next_record();
...
function drop(sub_code){
	if(confirm('삭제하시겠습니까?')){
		location.href="?mode=drop&code=<?=$code?>&sub_code="+sub_code;
	}
}
...
?>
$(function(){
	alert("상품관리에서 \"객실이름\"을 먼저 입력해 주세요");
	self.close();
});
<?
}
...
//-->
    ...
    <table border="0" cellspacing="1" cellpadding="3" width="95%" align="center">
		...
		for($i=0;$i<count($arr);$i++){
			...
			$sql2 = "select * from ez_hotel_room_jeju where code=$code and sub_code=$sub_code and subject='$arr[$i]' ";
			$dbo2->query($sql2);
	...
	<br>
<?
$sql = "select * from $table where code='$code' group by period_s,period_e order by period_s asc, period_e asc";
...
?>
	    <tr align='center' onMouseOver="this.style.backgroundColor='#EEEEFF'" onMouseOut="this.style.backgroundColor='#FFFFFF'" class="lh">
          ...
          <td align="center">
			...
			<span class="btn_pack small bold"><a href="?sub_code=<?=$rs[sub_code]?>&code=<?=$code?>&period_s=<?=$rs[period_s]?>&period_e=<?=$rs[period_e]?>"> 수정 </a></span>&nbsp;
			<span class="btn_pack small bold"><a href="?mode=copy&sub_code=<?=$rs[sub_code]?>&code=<?=$code?>"> 복사 </a></span>&nbsp;
