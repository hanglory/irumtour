...
if ($mode=="save"){
	...
	for($i = 0; $i < count($check);$i++){
		...
		if($rs[filename2]) @unlink("../../public/banner/$rs[filename2]");
		if($rs[filename3]) @unlink("../../public/banner/$rs[filename3]");
...
function chkForm(){
    ...
    <table border="0" cellspacing="1" cellpadding="3" class="viewWidth" width="750">
        ...
        <tr>
          ...
          <td>
            <input class="box" type="text" name="url" value="<?=$rs[url]?>" size="50" maxlength="190">
          </td>
        </tr>
	...
	<?if($rs[filename]):
		...
		?>
        <tr>
          <td class="subject">메인 이미지<br />729 * 360</td>
          <td height="20">
	...
	<?else:?>
		<tr>
          <td  class="subject">메인 이미지</td>
          <td height="20">
            <input class="box" type="file" name="file1" size=40> 729 * 360
          </td>
        </tr>
        <tr><td colspan="2" class="tblLine"></td></tr>
	<?endif;?>
   	...
   	<?if($rs[filename2]):
		...
		?>
        <tr>
          <td class="subject">버튼 off<br />236 * 90</td>
          <td height="20">
			...
			<div style="padding:15px 0 15px 0"><img src="../../public/banner/<?=$rs[filename2]?>"></div>
          </td>
        </tr>
        <tr><td colspan="2" class="tblLine"></td></tr>
	<?else:?>
		<tr>
          <td  class="subject">버튼 off</td>
          <td height="20">
            <input class="box" type="file" name="file2" size=40> 236 * 90
          </td>
        </tr>
        <tr><td colspan="2" class="tblLine"></td></tr>
	<?endif;?>
	...
	<?if($rs[filename3]):
		@$fileSize = filesize("../../public/banner/${rs[filename3]}");
		?>
        <tr>
          <td class="subject">버튼 on<br />236 * 90</td>
          <td height="20">
            ...
            <a class=soft href="../../include/download.php?file=<?=$rs[filename3]?>&size=<?=$fileSize?>" onFocus="blur(this)"><?=$rs[filename3]?> (<?=ceil($fileSize/1024)?>KB)</a>
			<div style="padding:15px 0 15px 0"><img src="../../public/banner/<?=$rs[filename3]?>"></div>
          </td>
        </tr>
        <tr><td colspan="2" class="tblLine"></td></tr>
	<?else:?>
		<tr>
          <td  class="subject">버튼 on</td>
          <td height="20">
            <input class="box" type="file" name="file3" size=40> 236 * 90
          </td>
        </tr>
        <tr><td colspan="2" class="tblLine"></td></tr>
	<?endif;?>
