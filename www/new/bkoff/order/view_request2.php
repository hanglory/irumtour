...
if($mode=="save"){
	...
	$sql = "select * from ez_request2 where id_no=$id_no";
	$dbo->query($sql);
	$rs=$dbo->next_record();
	if($rs[id_no]){
...
<?include("../top.html");?>
      ...
      <table border="0" cellspacing="1" cellpadding="3" width="750">
        ...
        <tr><td colspan="4" class='bar'></td></tr>
        <tr>
          <td class="subject" width="20%">행사내용</td>
          <td>
		    <?=$rs[etc6]?>
          </td>
        ...
        <tr><td colspan="4" class='bar'></td></tr>
        <tr>
          <td class="subject" width="20%">식사 수준</td>
          <td>
			<?=$rs[etc3]?>
          </td>

          <td class="subject" width="20%">객실타입</td>
          <td>
			<?=$rs[etc7]?>
          </td>
        </tr>
