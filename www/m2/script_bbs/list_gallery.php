	...
	for($i=0; $i<$total_col;$i++){
		...
		if($rs[id_no]){
			...
			$filename = "/new/public/bbs_files/".$rs[filename];
			$thumb = "/new/public/thumb/tb_".$rs[filename];
	?>

				<td width="<?=ceil(100/$EZ_BOARD_CONFIG_GALLERY_1ROWS)?>%">
					<a href="<?=$url?>&l=1" onFocus='blur(this)'><img src="<?=thumbnail($filename, $pic_width, $pic_width, 0, 1, 100, 0, "", "", $thumb)?>" class="gallery_img" onerror="this.src='../public/ez_board/bg/<?=$DEFAULT_IMG?>'" width="90%"></a>
					<div><?=$rs[subject]?></div>
				</td>
	<?
