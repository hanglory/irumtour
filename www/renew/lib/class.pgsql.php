...
class baseStaticPgsql {
	...
	function connect($host, $user, $pass, $base, $connect) {
		if ("$host$user$pass$base$connect") {
			...
			if ("$host$user$pass$base" != $this->Conn["host"].$this->Conn["user"].$this->Conn["pass"].$this->Base) {
				...
				$this->Conn["id"] = $connect("user=$user password=$pass dbname=$base");
				$this->Base = $base;
			}
		}
		return $this->Conn["id"];
	...
	function query($rid, $query) {
		$this->free($rid);
		if ($rid=@pg_query($this->Conn["id"], $query))
			$this->RIDS[$rid] = true;
		return $rid;
	}
}
...
class basePgsql {
	...
	function prvNumRows($query) {
		if (eregi("^[[:space:]]*SELECT[[:space:]]", $query))
			return @pg_num_rows($this->RID);
		else
			return @pg_affected_rows($this->RID);
	}
	...
	function query($query) {
		...
		if ($this->prvQuery($query)) {
			$this->Row = 0;
			return array($this->prvNumRows($query), @pg_num_fields($this->RID));
		}
		return false;
	}

	function next_record() {
		if ($this->Row >= pg_num_rows($this->RID))
			return false;
