...
<div id="ez_conents_wrap">
	...
	<div id="ez_view-attach">
		<img src="images/ez_board/ico_file.gif" alt="file icon"> <a href="include/download.php?file=<?=$rs[filename]?>&orgin_file_name=<?=$rs[filename_real]?>&dir=public/bbs_files"><?=$rs[filename_real]?> (<?=round(filesize("../new/public/bbs_files/".$rs[filename])/(1024))?> KB)</a>&nbsp;&nbsp;&nbsp;

		<?if($rs[filename2]){?>
		<img src="images/ez_board/ico_file.gif" alt="file icon"> <a href="include/download.php?file=<?=$rs[filename2]?>&orgin_file_name=<?=$rs[filename2_real]?>&dir=public/bbs_files"><?=$rs[filename2_real]?> (<?=round(filesize("../new/public/bbs_files/".$rs[filename2])/(1024))?> KB)</a>&nbsp;&nbsp;&nbsp;
		<?}?>

		<?if($rs[filename3]){?>
		<img src="images/ez_board/ico_file.gif" alt="file icon"> <a href="include/download.php?file=<?=$rs[filename3]?>&orgin_file_name=<?=$rs[filename3_real]?>&dir=public/bbs_files"><?=$rs[filename3_real]?> (<?=round(filesize("../new/public/bbs_files/".$rs[filename3])/(1024))?> KB)</a></li>
		<?}?>
	</div>
	<?}?>
	<!-- //ez_view-attach -->
	...
	<!-- button -->
	<div id="ez_view_btn"  style="padding-bottom:20px">
		<ul>
	...
	?>
	<div id="ez_view_reply" class="left" >
		...
		<div>
			<span class="bold">이름</span> <input type="text" name="name" size="10" maxlength="30" onkeyup="check_kor(this)"> (한글만 가능)
			<?if(!$_SESSION[sessMember][id]){?> &nbsp;&nbsp;
			<span class="bold">암호</span> <input type="password" name="pwd" size="10" maxlength="30" value="" class="input-text">
		<?}?>
		</div>
	...
	if($rows){
	?>
	<div id="ez_view_reply_list" class="left">
