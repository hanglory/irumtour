...
if($fname and $fname != "none"):		//업로드할 파일을 선택한 경우에만 작동
	...
	$fname_name_ex=explode("." , $fname_name);
	$upfile=$filename . '.' . $fname_name_ex[count($fname_name_ex)-1];
	$exist=file_exists("$path/$upfile");
	if($exist):
		@unlink("$path/$upfile");
	endif;
	...
	if($fname_type=="image/pjpeg" || $fname_type=="image/gif" || $fname_type=="image/png")):
	...
	endif;//if($fname_type=="image/pjpeg" || $fname_type=="image/gif" || $fname_type=="image/png")):
