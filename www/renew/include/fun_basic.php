...
reset($_POST);$chk_bid=0;while(list($key,$val)=each($_POST)){if($key=="content"){$_POST[$key] = secu($_POST[$key],1);}else{$_POST[$key] =(is_array($_POST[$key]))? $_POST[$key] : secu($_POST[$key]);}}reset($_POST);
...
Function getDow($date){
	...
	$result = substr(convertWeek($dow),0,2);
...
function html_input($name,$size,$maxlength,$class="box",$function="",$title="",$tabindex=''){
    ...
    if(strstr($class,"numberic")) $type="tel";
...
function get_sum($oid){
	...
	If($adult){
		$sql = "update ez_order set adult=$adult,price=(price_origin*$adult)+tour_option_price  where oid=$oid limit 1";
		$dbo->query($sql);
	}
...
function check_content_https($str,$bit=0){
    if($bit==1){
        $str = str_replace("http://irumtour.net","https://irumtour.net",$str);
        $str = str_replace("http://www.irumtour.net","https://irumtour.net",$str);
    }else{
